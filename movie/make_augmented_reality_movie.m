function [] = make_augmented_reality_movie()
%% make_augmented_reality_movie: 
%
...
heading = centroid + body_radius*[sind(B) -cosd(B)];
close
...
dim = size(vid);
pat_radius = floor(max(dim(1:2))/1.7);
pat_thickness = 6;
...
ax(1,1) = subplot(1,16,1:5); cla ; hold on; axis image
...
ax(1,2) = subplot(1,16,8:16); cla ; hold on
    ...
    elseif gamma == 0
        H.V = animatedline('Color', cc.V, 'LineWidth', lw);
        H.R = animatedline('Color', cc.R, 'LineWidth', lw, 'LineStyle', '--');
        H.B = animatedline('Color', cc.B, 'LineWidth', lw);
    elseif gamma > 0
        H.V = animatedline('Color', cc.V, 'LineWidth', lw);
        H.B = animatedline('Color', cc.B, 'LineWidth', lw);
...
for n = 1:n_frame
    set(fig, 'CurrentAxes', ax(1))
        ...
        delete(H.heading)
        H.heading(1) = plot([centroid(n,1) heading(n,1)], [centroid(n,2) heading(n,2)], '-', ...
            'Color', cc.B, 'LineWidth', 4, 'MarkerSize', 7);
     	H.heading(2) = plot(centroid(n,1), centroid(n,2), '.', ...
