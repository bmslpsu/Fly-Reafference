...
sacd.true_thresh = 300;
sacd.sacd_length = 0.2;
sacd.pks = [];
sacd.min_pkdist = 0.1;
...
for n = 1:N.file
    ...
    DATA.camera_times{n} = diff(vid_time(1:round(Fs*TimePeriods(D.trial(n)))));
    ...
    body_filt_low = filtfilt(b_low, a_low, body_scd.position);
    body_filt = filtfilt(b_high, a_high, body_filt_low);
    ...
    % Body raw coherence with stimulus
    fv = 0:0.02:50;
    time_stop = round(TimePeriods(1)*Fs);
    body_cohr = mscohere(func(1:time_stop), body_filt(1:time_stop), [], [], fv, Fs)';
    ...
    DATA.body_raw{n} = body_filt_low - body_filt_low(1);
    DATA.body_raw_coherence{n} = body_cohr;
    ...
    H.phase(H.phase > deg2rad(50)) = H.phase(H.phase > deg2rad(50)) - deg2rad(360); % correct phase wrapping
...
ylabel('Coherence')
xlabel('Frequency (hz)')
set(ax, 'XScale', 'log', 'XLim', [0.3 1], 'XTick', [0.3 0.5 1 3])
...
fig = figure (2) ; clf
set(fig, 'Color', 'w', 'Units', 'inches', 'Position', 1.8*[2 2 N.fly*1.2 5])
movegui(fig, 'center')
clear ax h
